...
def generisi_datume():
    ...
    for i in range(BROJ_DANA):
        ...
        if dan.weekday() != 6:  # izbaci nedelju
            datumi.append(dan.strftime("%Y-%m-%d"))
    return datumi

def generisi_slotove_za_dan(datum_str):
    """Kreira slotove za dati dan ako ne postoje, bez brisanja postojećih."""
    dan = datetime.strptime(datum_str, "%Y-%m-%d")
...
def osvezi_termine():
    """Osveži slotove za sve dostupne dane (dodaje nedostajuće)."""
...
def proveri_slotove_za_uslugu(datum, pocetak, trajanje):
    """Proverava da li su svi slotovi u opsegu slobodni i vraća listu slotova ili None."""
    ...
    if zauzeti:
        return None  # neki slot je zauzet
...
def rezervisi_slotove(datum, slotovi, ime, telefon, usluga, cena, trajanje):
    """Rezerviše listu slotova za datog klijenta."""
    conn = sqlite3.connect('termini.db')
    c = conn.cursor()
...
with tab1:
    ...
    if st.session_state['booking_success']:
        ...
        if datumi_raw:
            osvezi_termine()  # ovo sada samo dodaje nedostajuće slotove
                ...
                if st.session_state['izabrani_termin'] is not None:
                    ...
                    with st.form(key="klijent_form"):
                        ...
                        if slotovi is None:
                            ...
                            if potvrdi:
                                if ime and telefon and ime.strip() and telefon.strip():
                                    # Ponovna provera pre rezervacije (zbog mogućih konkurentnih zahteva)
                                    slotovi = proveri_slotove_za_uslugu(datum, kliknuto_vreme, usluga['trajanje'])
                                    if slotovi is None:
...
with tab2:
    ...
    if not st.session_state.admin:
        ...
        if datumi_raw:
            ...
            st.info("📭 Nema dostupnih datuma.") streamlit as st
import sqlite3
import os
from datetime import datetime, timedelta
